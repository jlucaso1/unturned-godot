...
Goal order is **parity first, then performance** — a performance HUD (`F3`) is on hand so the numbers stay
in view.
...
Export secrets, if you ever add any, land in `export_credentials.cfg`, which is git-ignored — never commit it.
...
bare machine — that is what CI runs, on Linux, Windows and macOS.

If your Godot came from a distro package and its exact `GodotSharp`/`Godot.NET.Sdk` version is not on
nuget.org yet, register the local nupkg folder per machine (never in the repo's `nuget.config` — a missing
