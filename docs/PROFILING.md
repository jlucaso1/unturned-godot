...
`$GODOT` below is your Godot 4.7 .NET binary. Most of this is Linux tooling — the two benchmark tiers
...
- **RAM**: `heaptrack --record-only -o /tmp/ht "$GODOT" -- --benchmark --gpu`, then `heaptrack_print
  ...
  symbolicate — read `/proc/<pid>/smaps_rollup` (RSS, `Private_Dirty`) for a live breakdown, and attribute
  per subsystem by differencing runs rather than by stack.
...
- **No sound**: gamescope hides the window but the audio still plays on your desktop — pass
