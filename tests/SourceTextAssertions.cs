...
public class SourceTextAssertions
{
    ...
    public void LegacyObjectIdsAreNormalizedBeforeEveryGuidKeyedWorldBuild()
    {
        ...
        // The policy is REQUIRED rather than defaulted, and that is load-bearing. It used to default to
        // the map's own answer, which read well and silently handed the editor a runtime calendar — the
        // dock warming December's variants while the preview asked for the authored originals. A default
        // is what let that regress without the compiler asking; without one, every call site has to say
        // which calendar it means, on a line a reviewer can see.
        Assert.DoesNotContain("HolidayPolicy? holidays = null", plan);
        Assert.DoesNotContain("holidays ?? HolidayPolicy", plan);
        Assert.Contains("HolidayPolicy holidays)", plan);

        // And nothing else derives the sequence. A second ResolvePlacementGuids call site is a fifth copy
        ...
        // The two runtime builds resolve the map's own calendar, redirects and all. Level.cs:282 gates
        // redirects on `isEditor == false`, so the editor must not — and both of its paths, the preview
        // and the dock's cache probe, go through ONE named policy, because the failure mode is those two
        // disagreeing and the dock extracting variants the preview then draws as fallback boxes.
        foreach (string source in new[] { File.ReadAllText(worldPath), File.ReadAllText(streamerPath) })
            Assert.Contains("HolidayPolicy.ForMap(", source);
        Assert.DoesNotContain("HolidayPolicy.ForMap(", preview);
        Assert.Contains("HolidayPolicy EditorHolidays => HolidayPolicy.FromClock()", preview);
        Assert.Equal(2, CountOccurrences(preview, "EditorHolidays)"));
    }

    // The cold load's three reads are independent, and the bundle decode cannot start until all three are
    // in — so running them together is the difference between waiting for the slowest and waiting for the
    // sum. Measured on Russia: 49 ms parallel against 90 ms sequential, and the placement read is the one
    // that scales with the map, so a workshop map placing ~100k objects is where it really tells.
    //
    // This is asserted in the source because nothing else can see it. A version that did the reads inside
    // LevelContentPlan.Resolve was correct, passed every gate in this repository, and quietly serialised
    // the whole phase; only reading the code showed it.
    [Fact]
    public void TheColdLoadsThreeIndependentReadsStillRunTogether()
    {
        string path = FindRepositoryFile(Path.Combine("src", "Rendering", "ObjectStreamer.cs"));

        string source = File.ReadAllText(path).Replace("\r\n", "\n");
        int start = source.IndexOf("private void LoadPlacements()", StringComparison.Ordinal);
        Assert.True(start >= 0, "LoadPlacements is where the cold load's parallel phase lives");
        string body = source[start..];

        // Each of the three on its own task, and the resolve only after all three have landed.
        Assert.Contains("Task.Run(() => placements = LevelContentPlan.ReadPlacements(_level))", body);
        Assert.Contains("Task.Run(() => _db = ContentExtraction.ScanAssets(_sources))", body);
        Assert.Contains("LevelContentPlan.ResolveFoliage(_sources, foliageGuids)", body);

        int waitAll = body.IndexOf("Task.WaitAll(placementFiles, assets, foliage)", StringComparison.Ordinal);
        int resolve = body.IndexOf("LevelContentPlan.Resolve(_sources", StringComparison.Ordinal);
        Assert.True(waitAll >= 0, "all three reads have to be waited on together");
        Assert.True(resolve > waitAll, "the resolve consumes all three, so it comes after the barrier");

        // And the foliage asset scan is inside the foliage task rather than after the barrier: it depends
        // on neither the placements nor the database, so it belongs beside them.
        int foliageTask = body.IndexOf("Task foliage = Task.Run(", StringComparison.Ordinal);
        int scan = body.IndexOf("LevelContentPlan.ResolveFoliage(", StringComparison.Ordinal);
        Assert.True(foliageTask >= 0 && scan > foliageTask && scan < waitAll,
            "resolving the foliage assets must stay inside the parallel phase");
    }
